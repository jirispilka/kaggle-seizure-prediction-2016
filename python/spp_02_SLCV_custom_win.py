...
from utils import compute_roc_auc_score_label_safe, load_features_and_preprocess, rename_groups_random, \
    TimeSeriesSplitGroupSafe, insert_pathol_to_normal_random_keep_order, probability_aggregate, \
    get_params_for_specific_clf
...
plabels_10min_aggregate_tr = d_tr['plabels_10min']

# print plabels_tr[0:150]
# print plabels_10min_aggregate_tr[0:150]
# sys.exit()
...
parameters = {'fs__k': [2, 5, 10, 20, 30, 40]}
...
for r in range(0, R):
    ...
    X, y, p, ind_for_resampling = insert_pathol_to_normal_random_keep_order(XTRAIN, ytrain, plabels_tr)
    p_10min_aggregate = ind_for_resampling[ind_for_resampling].copy()
    ...
    for param in param_grid:
        ...
        for i, (itrn, itst) in enumerate(skf.split(X, y, p)):
            ...
            p_agg_ts = p_10min_aggregate[itst]

            # sm = RandomOverSampler()
            # Xtr, ytr = sm.fit_sample(Xtr, ytr)
            # fs = SelectKBest(f_classif, k=param['fs__k'])

            # clf_param = get_params_for_specific_clf(param, clf.get_params())
            ...
            yts, yhat = probability_aggregate(yts, yhat, p_agg_ts, 'hmean')
            # print yhat.shape
...
print s
print df_split_results.groupby(s, as_index=False).agg({'result': ['median', 'sem']})

sns.pointplot(x='split', hue='fs__k', y='result', data=df_split_results, estimator=np.mean, ci=68)

# print df_results.describe()
# print df_results.info()
# print df_results

print df_results.groupby(parameters.keys(), as_index=False).agg({'result': ['mean', 'sem']})

sns.plt.figure()
sns.pointplot(x='fs__k', y='result', data=df_results, estimator=np.mean, ci=68)
