function spm_ut_analyse_1p_print_tex(X, aFeatNames, sTexName, y)
% Creates TeX file of features with table of AUC, Corr and p-values.
%
...
for i = 1:nNrFeat
    ...
    [~, auc, pvals] = spm_ut_analyse_single_feat_stat(x, y, '', name, 0);    
    if pvals < 0.05
        s = '\textsuperscript{*}';
    else
        s = '';
...
s = sprintf('%s\\begin{tabular}{ l  rl  rl}\n',s);
s = sprintf('%s\\toprule \n',s);
colnames = ' \multicolumn{2}{c}{AUC} & $p$';
s = sprintf('%s name & %s \\\\\n',s,colnames);
s = sprintf('%s\\midrule\n',s);
